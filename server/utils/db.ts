...
function atlasConnectionError(detail: string) {
  return createError({
    statusCode: 500,
...
export async function connectDb() {
  ...
  if (!uri) throw atlasConnectionError('La variable MONGODB_URI no está definida.')
  if (!cache.connection) {
    cache.connection = mongoose.connect(uri, { serverSelectionTimeoutMS: 5000 })
      ...
      .catch(error => {
        ...
        throw atlasConnectionError(detail)
      })
  }
